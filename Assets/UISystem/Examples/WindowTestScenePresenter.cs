...
namespace Game.UISystem.Example
{
    ...
    public class WindowTestScenePresenter : MonoBehaviour
    {
        ...
        public void OnDialogCompactClicked() =>
            Run(token => OpenFrame(UIWindowId.DialogCompactTest, "Dialog · 紧凑内容", token));
        public void OnDialogContentClicked() =>
            Run(token => OpenFrame(UIWindowId.DialogContentTest, "Dialog · 长内容自适应", token));
        public void OnSettingsClicked() => Run(OpenSettings);
        public void OnFullScreenInfoClicked() =>
            Run(token => OpenFrame(UIWindowId.FullScreenInfoTest, "FullScreen · 信息页", token));
        public void OnFullScreenListClicked() =>
            Run(token => OpenFrame(UIWindowId.FullScreenListTest, "FullScreen · 列表页", token));
        public void OnTipsClicked() => Run(OpenTips);
        public void OnNoneToastClicked() =>
            Run(token => OpenFrame(UIWindowId.NoneToastTest, "None · 轻提示", token));
        public void OnNoneLoadingClicked() =>
            Run(token => OpenFrame(UIWindowId.NoneLoadingTest, "None · 加载提示", token));
        ...
        private async UniTask OpenSettings(CancellationToken token)
        {
            ...
            await _ui.Open(UIWindowId.SettingWindow).Closed.AttachExternalCancellation(token);
